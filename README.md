...
Inside the experiment's folder, each patient/data will have its own folder. In default, there are 6 subfolders which are:
1. **1**: Contains age maps of each slice (normalised to 0-1) from the 1st level of hierarchy (1x1 patch). Saved in `.mat` format (Matlab file).
2. **2**: Contains age maps of each slice (normalised to 0-1) from the 2nd level of hierarchy (2x2 patch). Saved in `.mat` format (Matlab file).
3. **4**: Contains age maps of each slice (normalised to 0-1) from the 3rd level of hierarchy (4x4 patch). Saved in `.mat` format (Matlab file).
4. **8**: Contains age maps of each slice (normalised to 0-1) from the 4th level of hierarchy (8x8 patch). Saved in `.mat` format (Matlab file).
5. **IAM_combined_python**
6. **IAM_GPU_nifti_python**
