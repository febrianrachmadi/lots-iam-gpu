...
5. **IAM_combined_python**: Contains two sub-folders:
  1. **Patch** which contains visualisation of age maps of each slices in `JPEG` files, and
  2. **Combined** which contains visualisation of the final output of LOTS-IAM-GPU's computation in `JPEG` files and a `.mat` file.
